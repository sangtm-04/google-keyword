...
namespace Google.Ads.GoogleAds.Examples.V8
{
    ...
    public class GenerateKeywordIdeas : ExampleBase
    {
        ...
        public ArrayList MainKeyWords(string[] keywordTexts, string pageUrl, long[] locationIds, long languageId)
        {
            Options options = new Options();
            
            // The customer ID for which the call is made.
            options.CustomerId = long.Parse("3693114093");
            ...
            // information on determining this value, see
            // https://developers.google.com/google-ads/api/reference/data/codes-formats#languages.
            options.LanguageId = long.Parse("1040");

            // The list of seed keywords.
            // Add more items to the array as desired.
            options.KeywordTexts = keywordTexts;

            // The seed page URL.
            options.PageUrl = "https://amis.misa.vn/";

         

            GenerateKeywordIdeas codeExample = new GenerateKeywordIdeas();
            Console.WriteLine(codeExample.Description);
            return codeExample.Run(new GoogleAdsClient(), options.CustomerId, options.LanguageId, options.KeywordTexts.ToArray(),
                options.PageUrl);
        }
        ...
        public ArrayList Run(GoogleAdsClient client, long customerId, long languageId, string[] keywordTexts, string pageUrl)
        {
            ...
            try
            {
                ...
                ArrayList arlistKeyWords = new ArrayList();
                ...
                foreach (GenerateKeywordIdeaResult result in response)
                {
                    ...
                    arlistKeyWords.Add(new ReponseGoogleKeyWord {Text= result.Text,AvgMonthlySearches = metrics.AvgMonthlySearches, Competition = metrics.Competition.ToString() });;
                }

                return arlistKeyWords;
            }
